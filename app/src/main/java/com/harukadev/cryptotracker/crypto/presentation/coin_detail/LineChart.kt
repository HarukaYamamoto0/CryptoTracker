    ...
    Canvas(
        ...
        xLabelTextLayoutResults.forEachIndexed { index, result ->
            ...
            if (index == selectedDataPointIndex) {
                ...
                } + result.size.width / 2 // centers the text
        ...
        yLabelTextLayoutResults.forEachIndexed { index, result ->
            ...
            drawPoints = visibleDataPointsIndices.map {
                val x1 = viewPortLeftX + (it - visibleDataPointsIndices.first) *
                        xLabelWidth + xLabelWidth / 2f
                val ratio = (dataPoints[it].y - minYValue) / (maxYValue - minYValue)
                val y1 = viewPortBottomY - (ratio * viewPortHeightPx)
                DataPoint(
                    x = x1,
                    y = y1,
            ...
            for (i in 1 until drawPoints.size) {
                ...
                conPoints1.add(DataPoint(x1, y1, ""))
                conPoints2.add(DataPoint(x1, y2, ""))
            }
            ...
            drawPoints.forEachIndexed { index1, point ->
                if (isShowingDataPoints) {
                    ...
                    if (selectedDataPointIndex == index1) {
                        drawCircle(
                            color = Color.White,
                            radius = 15f,
                            center = circleOffset
